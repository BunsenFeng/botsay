...
if __name__ == "__main__":
    ...
    argParser.add_argument("--base_model", help="which base model was this finetuned with")
    argParser.add_argument("--tuned_model_name", help="name/path of the finetuned model")
    ...
    if not base_model == "chatgpt":
        tuned_model_name = "corpus/" + tuned_model_name
    ...
    for id in tqdm(test_ids):
        ...
        messages = instruction_tune_instance(node[id], approach)

        new_message = {"messages": [{"role": "user", "content": messages["messages"][0]["content"] + "\n" + messages["messages"][1]["content"]}]}
        ...
        if base_model == "chatgpt":
            ...
            prompt = new_message["messages"][0]["content"]
            input_ids = tokenizer.encode(prompt, return_tensors="pt")
            output = model.generate(input_ids, max_new_tokens=20, num_return_sequences=1, temperature=1.0, do_sample=True, pad_token_id=tokenizer.eos_token_id)
            response = tokenizer.decode(output[0], skip_special_tokens=True)

        preds.append(lm_utils.answer_parsing(response))
        probs.append(0.5) # PROBABLY NO PROBS

    print("------------------")
    print("Approach:", approach)
    print("Base model:", base_model)
    print("Model:", tuned_model_name)
